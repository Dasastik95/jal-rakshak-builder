...
// Fix default marker icons (Leaflet bundling issue with bundlers)
...
export function PropertyMap({ properties, height = 480 }: { properties: MapProperty[]; height?: number }) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border" style={{ height }}>
      <MapContainer center={center} zoom={withCoords.length ? 13 : 12} style={{ height: "100%", width: "100%" }} scrollWheelZoom={false}>
        <TileLayer
          ...
          <Marker key={p.id} position={[Number(p.latitude), Number(p.longitude)]}>
            <Popup>
              <div className="min-w-[180px]">
                ...
                <div className="text-xs text-gray-600">{[p.locality, p.city].filter(Boolean).join(", ")}</div>
                <div className="mt-1 font-bold" style={{ color: "hsl(35 100% 45%)" }}>{formatPrice(p.price, p.listing_type)}</div>
                <Link to="/properties/$id" params={{ id: p.id }} className="mt-2 inline-block text-xs text-blue-700 underline">View details →</Link>
              </div>
            </Popup>
          </Marker>
        ))}
